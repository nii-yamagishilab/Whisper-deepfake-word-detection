...
class TedXSpeechDataset(torch.utils.data.Dataset):
    def __init__(self, audio_info_list, tokenizer, sample_rate) -> None:
        ...
        # if "<|startoflm|>" not in self.tokenizer.special_tokens:
        #     self.tokenizer.special_tokens.append("<|startoflm|>")
    ...
    def __getitem__(self, id):
        ...
        # print(text)
        text = [*self.tokenizer.sot_sequence_including_notimestamps] + self.tokenizer.encode(text, allowed_special={"<|startoflm|>"})
        # print(text)
...
class WhisperModelModule(LightningModule):
    ...
    def train_dataloader(self):
        dataset = TedXSpeechDataset(self.__train_dataset, self.tokenizer, self.cfg.sample_rate)
    ...
    def val_dataloader(self):
        dataset = TedXSpeechDataset(self.__eval_dataset, self.tokenizer, self.cfg.sample_rate)
        return torch.utils.data.DataLoader(dataset, 
                          batch_size=self.cfg.batch_size, 
...
# Dataset setup
# DATASET_DIR_SPOOF = "/home/tran/dev/partial_spoof/tedx_test/tedx_fr_random_3k_vocoded_json"
# DATASET_DIR_BONAFIDE = "/home/tran/dev/partial_spoof/tedx_test/tedx_fr_random_3k_json"
# DATASET_DIR = "/home/tran/dev/partial_spoof/tedx_test/"
# # DATASET_DIR = "/home/tran/dev/partial_spoof/tedx/tedx_fr_random_3k_json"
...
BASE_DIR = "tedx_test_special_tokens"
print(f"\n{'#'*50}")
...
log_output_dir = "content_special_tokens_whisper_base_3e6/logs"
check_output_dir = "content_special_tokens_whisper_base_3e6/artifacts"

train_name = "whisper_base_finetune"
train_id = "all_finetune_base_finetune"
...
def extract_vocoding_labels(text: str):
    """
    Given a text with vocoded words prefixed by '<|startoflm|>',
    ...
    i = 0
    while i < len(words):
        if words[i] == "edaan" or words[i] == "<|startoflm|>":
            if i + 1 < len(words):
...
# -----------------------------
checkpoint_path = "content_special_tokens_whisper_small_3e6/artifacts/checkpoint/checkpoint-epoch=0007.ckpt"
...
def count_phrases_and_words(texts):
    ...
    for t in texts:
        ...
        num_words += sum(1 for w in words if w != "<|startoflm|>")
