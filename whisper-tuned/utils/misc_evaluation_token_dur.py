#!/usr/bin/env python
"""
also compute the token and chara duration
"""
import os
import re
import sys
...
tokenizer = whisper.tokenizer.get_tokenizer(True, language="fr", task=options.task)
...
def compute_detection_metrics(refs, preds, output_file, old_method=False):
    a = b = c = d = 0

    error_buf = []
    
    for idx, (ref_text, hyp_text) in tqdm(enumerate(zip(refs, preds)), total=len(refs)):
            ...
            for chunk in align:                
                ...
                if chunk.type == 'equal' or chunk.type == 'substitute':
                    ...
                    for ref_i, hyp_i in zip(range(ref_s, ref_e), range(hyp_s, hyp_e)):
                        ...
                        if gt == "bonafide" and pred == "bonafide":
                            a += 1
                            error_buf.append([len(ref_labels[ref_i][1]), tokenizer.encode(' '+ref_labels[ref_i][1]), 1, 0, 0, 0])
                        elif gt == "spoof" and pred == "spoof":
                            b += 1
                            error_buf.append([len(ref_labels[ref_i][1]), tokenizer.encode(' '+ref_labels[ref_i][1]), 0, 1, 0, 0])
                        elif gt == "bonafide" and pred == "spoof":
                            c += 1  # false negative
                            error_buf.append([len(ref_labels[ref_i][1]), tokenizer.encode(' '+ref_labels[ref_i][1]), 0, 0, 1, 0])
                        elif gt == "spoof" and pred == "bonafide":
                            d += 1  # false positive
                            error_buf.append([len(ref_labels[ref_i][1]), tokenizer.encode(' '+ref_labels[ref_i][1]), 0, 0, 0, 1])
                        else:
                    ...
                    for hyp_i in range(hyp_s, hyp_e):
                        ...
                        if pred == "spoof":
                            ...
                            c += 1
                            error_buf.append([len(hyp_labels[hyp_i][1]), tokenizer.encode(' '+hyp_labels[hyp_i][1]), 0, 0, 1, 0])
                        elif pred == "bonafide":
                            a += 1
                            error_buf.append([len(hyp_labels[hyp_i][1]), tokenizer.encode(' '+hyp_labels[hyp_i][1]), 1, 0, 0, 0])
                        else:
                            assert 1==0, "Impossible"
                else:
                    assert 1==0, "Impossible"
    ...
    with open(output_file, 'wb') as file_ptr:
        pickle.dump(error_buf, file_ptr)
...
def main(input_file, output_file):
    ...
    # -----------------------------
    metrics = compute_detection_metrics(refs, res, output_file)
